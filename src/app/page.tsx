          ...
          <div className="space-y-3">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-12">
                <svg className="h-8 w-8 animate-spin text-slate-400 dark:text-slate-500" fill="none" viewBox="0 0 24 24">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                </svg>
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                <div className="rounded-full bg-slate-200/50 dark:bg-slate-800/50 p-4 mb-4">
                ...
                <p className="text-slate-600 dark:text-slate-400 font-medium">No items yet</p>
              ...
              <ul className="space-y-2">
                {items.map((item) => (
                  <li
                    key={item.id}
                    className="group flex items-center justify-between rounded-xl bg-slate-100/30 dark:bg-slate-800/30 border border-slate-300/30 dark:border-slate-700/30 px-4 py-3 glass-card-interactive"
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <span className="text-slate-900 dark:text-white truncate">{item.name}</span>
                      <span className="text-xs text-slate-400 dark:text-slate-500 font-mono">#{item.id}</span>
                    </div>
                    <button
                      onClick={() => handleDelete(item.id)}
                      disabled={deletingId === item.id}
                      className="ml-3 rounded-lg px-3 py-1.5 text-sm text-slate-500 dark:text-slate-400 transition-all hover:bg-red-500/10 hover:text-red-500 dark:hover:text-red-400 focus:outline-none focus:ring-2 focus:ring-red-500/50 disabled:opacity-50 opacity-0 group-hover:opacity-100 focus:opacity-100"
